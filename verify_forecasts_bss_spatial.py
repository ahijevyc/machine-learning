...
#fcst_file  = './trained_models_paper/predictions_nn_40km_2hr_all'
fcst_file = 'predictions_nn_40km_2hr_NSC3km-12sec'
#fcst_file = '/glade/work/ahijevyc/NSC_objects/predictions_nn_40km_2hr_basic_neighborhood.0latlon_hash_buckets.2015043000-2017122900.train.20152016.mem1.predict.2017.mem9.pk'
#fcst_file = 'predictions_nn_40km_2hr_NSC3km-12sec_all'
...
#all_predictions = []
#for mem in range(3,4):
#  print(mem)
#  fcst_file = '/glade/work/ahijevyc/NSC_objects/predictions_nn_40km_2hr_basic_neighborhood.0latlon_hash_buckets.2015043000-2017122900.train.20152016.mem1.predict.2017.mem%d.pk'%mem
#  predictions_all_nn, labels_all, fhr_all, cape_all, shear_all, uh_all, uh120_all, uh01_all, dates_all =  pickle.load(open(fcst_file, 'rb'))
#  all_predictions.append(predictions_all_nn)
#predictions_all_nn = np.array(all_predictions)
#predictions_all_nn = np.mean(predictions_all_nn, axis=0)
...
#uh120_all = pickle.load(open('predictions_nn_40km_2hr_uh01_NSC1km_v2', 'rb'))
#uh120_all = np.zeros((predictions_all_nn.shape[0]))
#print(uh120_all.shape)
...
#predictions_gridded_uh, labels_gridded   = make_gridded_forecast((uh120_all>150).astype(np.int32), labels_all[:,hazard_idx], dates_all, fhr_all)
predictions_gridded_uh, labels_gridded   = make_gridded_forecast((uh120_all>75).astype(np.int32), labels_all[:,hazard_idx], dates_all, fhr_all)
...
# smooth ML forecast
#smooth_sigma = [0.5]
#predictions_gridded = smooth_gridded_forecast(predictions_gridded)[0]

### filter predictions by forecast hour
num_fhr = (efhr - sfhr) + 1
climo_all                            = climo_all[:,sfhr-1:efhr,:]
predictions_gridded, labels_gridded  = predictions_gridded[:,sfhr-1:efhr,:], labels_gridded[:,sfhr-1:efhr,:]
predictions_gridded_uh               = predictions_gridded_uh[:,sfhr-1:efhr,:]
predictions_gridded_uh_smoothed      = predictions_gridded_uh_smoothed[:,:,sfhr-1:efhr,:]
...
# these code blocks below compute spatial or daily BSS for fixed UH thresholds
...
plot_2d_hist(preduh.flatten(), predictions_all[:,hazard_idx])
