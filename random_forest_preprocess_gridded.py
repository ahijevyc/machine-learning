...
def get_closest_report_distances(grid_lats, grid_lons, grid_times): 
    ...
    sdate, edate = thisdate-timedelta(hours=4) - gmt2cst, thisdate+timedelta(hours=40) - gmt2cst
...
lons = np.repeat(lons[np.newaxis,:], 36, axis=0)
lats = np.repeat(lats[np.newaxis,:], 36, axis=0)
x_ind, y_ind = np.indices((65,93))
x_ind = np.repeat(x_ind[np.newaxis,:], 36, axis=0)
y_ind = np.repeat(y_ind[np.newaxis,:], 36, axis=0)

#available fields in upscaled files
#upscaled_fields = { 'UP_HELI_MAX':[], 'UP_HELI_MAX03':[], 'UP_HELI_MAX01':[], 'W_UP_MAX':[], 'W_DN_MAX':[], 'WSPD10MAX':[], 'STP':[], 'LR75':[], 'CAPESHEAR':[],
#           'MUCAPE':[], 'SBCAPE':[], 'SBCINH':[], 'MLCINH':[], 'MLLCL':[], 'SHR06': [], 'SHR01':[], 'SRH01':[], 'SRH03':[], 'T2':[], 'TD2':[], 'PSFC':[], 'PREC_ACC_NC':[], 'HAILCAST_DIAM_MAX':[], \
...
while thisdate <= enddate:
    ...
    else: fname = '/glade/work/sobash/NSC/%s_%s_upscaled.npz'%(thisdate.strftime('%Y%m%d00'), model)

    if os.path.exists(fname):
        data = np.load(fname, allow_pickle=True)
        upscaled_fields = data['a'].item() #have to use item since dictionary was stored 
  
        # add UH01-120km to dictionary 
        data = np.load('/glade/work/sobash/NSC/%s_%s_upscaled_UH01.npz'%(thisdate.strftime('%Y%m%d00'), model), allow_pickle=True)
        upscaled_fields.update(data['a'].item()) 
        ...
        upscaled_fields['UP_HELI_MAX-N1T5'] = scipy.ndimage.filters.maximum_filter(upscaled_fields['UP_HELI_MAX'], footprint=np.ones((5,1,1))) #max 4-hr UH within 40km
        upscaled_fields['UP_HELI_MAX80-N1T5'] = scipy.ndimage.filters.maximum_filter(upscaled_fields['UP_HELI_MAX80'], footprint=np.ones((5,1,1))) #max 4-hr UH within 80km
        upscaled_fields['UP_HELI_MAX120-N1T5'] = scipy.ndimage.filters.maximum_filter(upscaled_fields['UP_HELI_MAX120'], footprint=np.ones((5,1,1))) #max 4-hr UH within 120km
   
        upscaled_fields['UP_HELI_MAX01-N1T5'] = scipy.ndimage.filters.maximum_filter(upscaled_fields['UP_HELI_MAX01'], footprint=np.ones((5,1,1))) #max 4-hr UH within 40km
        upscaled_fields['UP_HELI_MAX01-120-N1T5'] = scipy.ndimage.filters.maximum_filter(upscaled_fields['UP_HELI_MAX01-120'], footprint=np.ones((5,1,1))) #max 4-hr UH within 120km
