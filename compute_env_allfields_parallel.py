...
def upscale(field, type='mean', maxsize=27):
    if model in ['NSC3km', 'NSC3km-12sec', 'GEFS', 'RT2020']: kernel = np.ones((26,26)) #should this be 27?
...
def get_closest_gridbox():
    ...
    gpfname = '/glade/work/sobash/NSC_objects/nngridpts_80km_%s'%model
...
def upscale_forecast(fhr): 
    ...
    try:
        ...
        if model == 'RT2020': wrffile = "/glade/scratch/wrfrt/realtime_ensemble/rt2020/%s/wrf_rundir/ens_%d/diags_d01_%s.nc"%(yyyymmddhh,mem,wrfvalidstr2)
        fh = Dataset(wrffile)
        ...
        for f in this_upscaled_fields.keys():
            ...
            if f in ['UP_HELI_MAX', 'UP_HELI_MAX03', 'UP_HELI_MAX01', 'W_UP_MAX', 'W_DN_MAX', 'WSPD10MAX', 'HAILCAST_DIAM_MAX']:
                if model in ['NSC3km-12sec', 'RT2020']: field_interp = upscale(this_field, type='max', maxsize=27)
                else: field_interp = upscale(this_field, type='max', maxsize=27*3)
            elif f in ['UP_HELI_MAX80']:
                if model in ['NSC3km-12sec', 'RT2020']: field_interp = upscale(this_field, type='max', maxsize=53)
                else: field_interp = upscale(this_field, type='max', maxsize=53*3)
            elif f in ['UP_HELI_MAX120', 'UP_HELI_MAX01-120']:
                if model in ['NSC3km-12sec', 'RT2020']: field_interp = upscale(this_field, type='max', maxsize=81)
                else: field_interp = upscale(this_field, type='max', maxsize=81*3)
            else:
                if model in ['NSC3km-12sec', 'RT2020']: field_interp = upscale(this_field, type='mean', maxsize=26) #should be 27?
...
else: np.savez_compressed('/glade/work/sobash/NSC/%s_%s_upscaled_UH01'%(sdate.strftime('%Y%m%d%H'),model), a=upscaled_fields)
