...
# compute these fields on 40km for reviewer comment
upscaled_fields = { 'UP_HELI_MAX':[], 'W_UP_MAX':[], 'WSPD10MAX':[], 'STP':[], 'LR75':[], 'CAPESHEAR':[], 'SBCAPE':[],  'SBCINH':[], 'MLLCL':[], 'SHR06': [], 'SHR01':[], 'SRH01':[], 'SRH03':[], 'T2':[], 'TD2':[], 'PREC_ACC_NC':[] }
 
#for fhr in range(37):
for fhr in range(12,37,6):
    ...
    try:
        ...
        for f in upscaled_fields.keys():
            ...
            if f in ['UP_HELI_MAX', 'UP_HELI_MAX03', 'UP_HELI_MAX01', 'W_UP_MAX', 'W_DN_MAX', 'WSPD10MAX', 'HAILCAST_DIAM_MAX']:
                if model in ['NSC3km-12sec']: field_interp = upscale(this_field, type='max', maxsize=27/2.0)
                ...
                if model in ['NSC3km-12sec']: field_interp = upscale(this_field, type='mean', maxsize=26/2.0) #should be 27?
...
else: np.savez_compressed('/glade/work/sobash/NSC/%s_%s_upscaled_40km'%(sdate.strftime('%Y%m%d%H'),model), a=upscaled_fields)
