...
def assertclose(df, *c, lsuffix="", rsuffix="", atol=0.1):
    ...
    for column in c:
        ...
        if xymean[lc].dtype == '<M8[ns]' and xymean[rc].dtype == '<M8[ns]':
            ...
            assert xymean[lc].equals(xymean[rc]), (
...
def configs_match(ylargs, args):
    ...
    assert args.trainstart <= trainstart, (
...
def get_argparser():
    parser = argparse.ArgumentParser(
        description="train/test dense neural network", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--batchnorm', action='store_true',
                        help="use batch normalization")
    parser.add_argument('--batchsize', type=int, default=1024,
                        help="nn training batch size")  # tf default is 32
    parser.add_argument("--clobber", action='store_true',
                        help="overwrite any old outfile, if it exists")
    ...
    parser.add_argument('--fits', nargs="+", type=int, default=None,
                        help="work on specific fit(s) so you can run many in parallel")
    parser.add_argument('--flash', type=int, default=10,
                        help="GLM flash count threshold")
    parser.add_argument('--folds', nargs="+", type=int, default=None,
                        help="work on specific fold(s) so you can run many in parallel")
    parser.add_argument('--kfold', type=int, default=5,
                        help="apply kfold cross validation to training set")
    parser.add_argument(
    ...
    parser.add_argument('--model', choices=["HRRR", "NSC1km", "NSC3km-12sec",
                        "NSC15km"], default="HRRR", help="prediction model")
    parser.add_argument('--neurons', type=int, nargs="+",
                        default=[16, 16], help="number of neurons in each nn layer")
    parser.add_argument('--nfits', type=int, default=5,
                        help="number of times to fit (train) model")
    parser.add_argument('--nprocs', type=int, default=0,
                        help="verify this many forecast hours in parallel")
    parser.add_argument('--optimizer', type=str,
                        choices=['Adam', 'SGD'], default='Adam', help="optimizer")
    parser.add_argument('--reg_penalty', type=float,
                        default=0.01, help="L2 regularization factor")
    parser.add_argument(
    ...
    parser.add_argument('--suite', default='default',
                        help="name for suite of training features")
...
def get_optimizer(s, learning_rate=0.001, **kwargs):
    if s == 'Adam':
        ...
        momentum = 0.99
        nesterov = True
        o = optimizers.SGD(learning_rate=learning_rate,
...
def get_ifiles(args, idir):
    ...
    if model == "HRRR":
        ...
        if args.debug:
            search_str = search_str.replace(
                "HRRRX", "HRRR").replace("20*", debug_replace)
        else:
    ...
    if idate:
        if model == "HRRR":
            assert idate >= pd.to_datetime(
                "20191002"), f"No {model} before 20191002"
...
def load_df(args, idir="/glade/work/sobash/NSC_objects",
    ...
    fmt = "%Y%m%d%H-%M%S.par"
    earliest_valid_time = (
        datetime.datetime.strptime(os.path.basename(
            ifiles[0])[-19:], fmt) + pd.Timedelta(hours=1)
    )
    latest_valid_time = (
        datetime.datetime.strptime(os.path.basename(
            ifiles[-1][-19:]), fmt) + pd.Timedelta(hours=48)
    )
    df = engine.read_parquet(ifiles, engine="pyarrow")

    df = df.rename(
        columns=dict(xind="y", yind="x",
    ...
    for rptdist in [20, 40]:
        ...
        name_dict = {s: f"{s}_{rptdist}km_{twin}hr" for s in [
            "cg", "ic", "cg.ic"]}
    ...
    all_ltg = xarray.merge(
        [ltg_sum_coarse, wbug_dict[40], glm20, glm40], compat="override").to_dataframe()
    ...
    if sanity_check:
        ll = ["lat", "lon"]
        xymean = df[["lat", "lon", "lat_y", "lon_y", "y", "x"]].groupby([
                                                                        "y", "x"]).mean()
        assert np.allclose(
            xymean[ll], xymean[[l+"_y" for l in ll]], atol=0.289)
    ...
    df = df.dropna()
    logging.warning(
        f"kept {len(df)}/{beforedropna} {len(df)/beforedropna:.0%} rows")
...
def rptdist2bool(df, args):
    ...
    for rptdist in [20, 40]:
        ...
        assert not df[labels].isna().any(
            axis=None), f'label(s) missing {df[labels].isna().any()}'
        lsrtypes = ["sighail", "sigwind", "hailone",
                    "wind", "torn", "windmg", "svrwarn", "torwarn"]
        oldtwin = [0, 1, 2]
        logging.warning(
        ...
        new_label_cols = [
            r.replace("rptdist", f"{rptdist}km") for r in label_cols]
        logging.info(
    ...
    if date:
        ...
        if oneGLMfile:
            ...
            if os.path.exists(ifile):
                ...
                logging.error(
                    f"{ifile} does not exist. To create, concatenate GLM files:")
        ...
        assert (glm.time[1] - glm.time[0]) == np.timedelta64(3600,
                                                             's'), 'glm.time interval not 1h'
...
def upscale(field, nngridpts, type='mean', maxsize=27):
    if type == 'mean':
        field = scipy.ndimage.filters.uniform_filter(
            field, size=maxsize, mode='nearest')
...
def read_csv_files(sdate, edate, dataset, members=[str(x) for x in range(1, 11)], columns=None):
    ...
    all_files = [x for x in all_files if sdate.strftime(
        '%Y%m%d') <= x[-17:-9] <= edate.strftime('%Y%m%d')]
    all_files = sorted(all_files)  # important for predictions_labels output
    logging.debug(
        f'Reading {len(all_files)} forecasts from {sdate} to {edate}')
    ...
    ext = all_files[0][-4:]
    if ext == ".csv":
        df = pd.concat((pd.read_csv(f, engine='c')
                       for f in all_files), sort=False)
    elif ext == ".par":
...
def normalize_multivariate_data(data, features, scaling_values=None, nonormalize=False):
    ...
    for i in range(data.shape[-1]):
        ...
        normed_data[:, i] = (data[:, i] - scaling_values.loc[features[i],
                             "mean"]) / scaling_values.loc[features[i], "std"]
